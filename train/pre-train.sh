...
# lr
[[ -z ${warmup_updates} ]] && warmup_updates=8000
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES} fairseq-train ${data_path} \
  ...
  ${dropout_cmd}  \
  --optimizer adam --adam-betas '(0.9, 0.997)' --clip-norm ${clip_norm} \
...
if [[ $? == 0 ]]; then
    echo "pre-train successfully finished" >&2
else
